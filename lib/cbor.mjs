...
const PHASES = ['BEGIN', 'ITEM', 'FINISH', 'ERROR']

export function float16(nval) {
  const sign = (nval & 0x8000) ? -1 : 1
  const exp = (nval & 0x7C00) >> 10
  const mant = nval & 0x03ff
  console.log({sign, exp, mant})
  if (!exp) {
    // subnormal
    return sign * 5.9604644775390625e-8 * mant // 5.9...e-8 = 2^-24
  } else if (exp === 0x1f) {
    // Nan/+-Infinity
    return sign * (mant ? NaN : Infinity)
  }
  // 25 = 15 of bias plus 10 to move the 1 to the right place
...
export class Decoder {
  ...
  _event(mt, bytes, phase, line) {
    ...
    if (this.opts.verbose) {
      console.log('%O', {
        mt, bytes, val, phase: PHASES[phase], line, stack: this.stack
      })
    }
    if (mt === this.mod.FAIL) {
    ...
    switch (mt) {
      ...
      case 7: {
        switch (bytes) {
          ...
          case 8: {
            // If everyone has BigUint64Array
            // this.u64[0] = val
            // ;[ret] = this.f64
